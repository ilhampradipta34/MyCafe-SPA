...
export default function KasirTransDashboard() {
  ...
  const fetchTransactions = async () => {
    ...
    const { data, error, count } = await supabase
      .from("orders")
      .select(
        "*, users(id, name, role), order_items(id, order_id, product_id, quantity, price_each), payments(id, order_id, amount, payment_method, paid_at)",
        { count: "exact" }
      )
      .eq("user_id", userId) // hanya data kasir login
  ...
  useEffect(() => {
    ...
    searchTimeout.current = setTimeout(async () => {
      ...
      const { data, error } = await supabase
        ...
        .eq("user_id", userId); // ⬅️ filter milik kasir
    ...
    },
    {
      ...
      render: (row) => {
        return row.status === "paid" ? (
          ...
          <button className="!text-white !bg-yellow-400 rounded-md shadow-md p-1.5">
            {row.status}
          </button>
        ) : (
